...
app.use(session({secret:'mysecret',   resave: false, saveUninitialized: true, cookie:{secure:false, maxAge: 1000*60*60*24*30 }}));
...
// 	let suppliers= await db.select('*','suppliers');
// 	let admins= await db.select('*','admins');

// 	let obj={users, suppliers, admins};
	
// 	db.foreach(obj, function(table_name, table){
// 		db.foreach(table, function(row_index, row){
// 			var insertable={local_id:row.id, email:row.email, password:row.password, table_name: table_name}
// 			if(row.fb_id!=undefined){insertable.fb_id=row.fb_id;}
// 			db.insert("credentials", insertable);
// 		})
// 	})
// 	res.send('goood');
// });
...
app.get(`/firebase-messaging-sw.js`, (_,res) => res.sendFile(resolve(__dirname, "public/js", "firebase-messaging-sw.js")));


app.post(`/ajax_supplier/:id`, async (req,res,next) => {
	let result = {};
	let func_name = req.params.id;
	let data = req.body;		
	if(func_name == 'load_orders'){
		let {supplier_id} = data;
		result = await db.select('*', 'orders', {supplier_id});
	}
	res.send(result);
});


app.post(`/ajax/:id`, async (req, res, next) => {
	let result = {};
	let func_name = req.params.id;
	let data = req.body;	
	...
	try{
		...
		if(func_name == 'load_orders'){
			result['orders']= await db.select('*', 'orders', {user_id:data.user_id, deleted:0});
		}
